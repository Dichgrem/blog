...
我们选择清华源，速度较快。注意：Debian 安装时默认开启安全源，这个源是国外的所以下载速度极慢，因此还需要修改配置文件，这里使用 Ctrl+Alt+F2 从图形界面转到tty命令终端, 键入 Enter，使用命令``nano /target/etc/apt/sources.list``，将所有的源都改成 http://mirrors.ustc.edu.cn 或者清华源，然后 Ctrl+X 退出保存， Ctrl+Alt+F5 回到图形界面。
